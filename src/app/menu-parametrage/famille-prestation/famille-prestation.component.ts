...
@Component({
  selector: 'app-famille-prestation',
  templateUrl: './famille-prestation.component.html',
  styleUrls: ['./famille-prestation.component.css', '.../../../src/assets/css/newStyle.css'
    , '.../../../src/assets/css/StyleApplication.css'], providers: [ConfirmationService, MessageService]
})
export class FamillePrestationComponent implements OnInit {
  ...
  ListTypeFamillePrestation = new Array<any>();
  selectedTypeFamillePrestation: any = '';
  ...
  ngOnInit(): void {
    this.items = [
      { label: this.i18nService.getString('LabelActif') || 'LabelActif', icon: 'pi pi-file-check', command: () => { this.GetAllFamillePrestationActif() } },
      { label: this.i18nService.getString('LabelInActif') || 'LabelInActif', icon: 'pi pi-file-excel', command: () => { this.GetAllFamillePrestationInActif() } },
      { label: this.i18nService.getString('LabelAll') || 'LabelAll', icon: 'pi pi-file', command: () => { this.GetAllFamillePrestation() } },
    ];
    this.activeItem = this.items[0];
    this.GetColumns();
    this.GetAllFamillePrestationActif();
  }
  ...
  GetCodeSaisie() {
    this.param_service.GetCompteur("CodeSaisieFamillePrestation").
      subscribe((data: any) => {
  ...
  DeleteFamillePrestation(code: any) {
    this.param_service.DeleteFamillePrestation(code).subscribe(
      (res: any) => {
  ...
  public onOpenModal(mode: string) {
    ...
    if (mode === 'add') {
      ...
      this.formHeader = this.i18nService.getString('Add') + ' ' + this.i18nService.getString('FamillePrestation');
      this.onRowUnselect(event);
    ...
    if (mode === 'edit') {
      ...
      if (this.code == undefined) {
        ...
        button.setAttribute('data-target', '#Modal');
        this.formHeader = this.i18nService.getString('Modifier') + ' ' + this.i18nService.getString('FamillePrestation');;
    ...
    if (mode === 'Delete') {
      ...
      if (this.code == undefined) {
...

        {
          ...
          this.formHeader = this.i18nService.getString('Delete') + ' ' + this.i18nService.getString('FamillePrestation');;
          this.visDelete = true;
    ...
    if (mode === 'Print') {
      if (this.code == undefined) {
        ...
        this.formHeader = "Imprimer Liste FamillePrestation"
  ...
  PostFamillePrestation() {
    ...
    if (isValid) {
      ...
      if (this.code != null) {
        ...
        this.param_service.UpdateFamillePrestation(body).subscribe(
        ...
        this.param_service.PostFamillePrestation(body).subscribe(
          (res: any) => {
            this.CtrlAlertify.PostionLabelNotification();
            this.CtrlAlertify.ShowSavedOK();
            this.visibleModal = false;
  ...
  GetAllFamillePrestationActif() {
    this.IsLoading = true;
    this.param_service.GetFamillePrestationActif(true).subscribe((data: any) => {
  ...
  GetAllFamillePrestationInActif() {
    this.IsLoading = true;
    this.param_service.GetFamillePrestationInActif(false).subscribe((data: any) => {
